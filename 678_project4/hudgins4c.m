...
rules = [F D A F F D D A A F D A];
all_plots_extra(2, 2, 0.5, rules)
q_centers = [0 : 100/1 : 100];
e_centers = [0 : 10/1 : 10];
f_centers = [50, 75, 100];
fuzzy_eval_extra(50, 0, 0, q_centers, e_centers, f_centers, 0.5, rules)
...
function all_plots_extra(num_q_centers, num_e_centers, overlap, y)
    ...
    subplot(1,4,3);
    plot_mfs(f_centers, overlap, 100)
    xlabel('F')
    ...
    figure
    fmesh(@(q, f) fuzzy_eval_extra(q, 0, f, q_centers, e_centers, f_centers, overlap, y ), [0 100 50 100])
    xlabel('Q')
    ylabel('F')
    zlabel('G')
    title('Q for fixed E')
