...
if isa(H,'double') && ~usrSuppliedHessMult
   if norm(H,'inf')==0 || isempty(H)
      % Really an LP problem
      warning(message('optim:quadprog:NullHessian'))
      % Set Algorithm to inprog's default and call linprog. Currently options.Algorithm
      % may be set to a quadprog algorithm that doesn't translate to linprog
...
elseif strcmpi(output.algorithm,trustRegReflect)
    ...
    if (hasIneqs || hasEqsAndBnds || hasMoreEqsThanVars || hasNoConstrs)
        error(message('optim:quadprog:ConstrTRR', ...
        ...
        if exitflag == -10  % Problem not handled by sqpmin at this time: dependent rows
            error(message('optim:quadprog:DepConstrTRR', ...
                addLink( 'Choosing the Algorithm', 'choose_algorithm' ))) 
        end
    end
elseif strcmpi(output.algorithm,activeSet)
   if isempty(X0)
